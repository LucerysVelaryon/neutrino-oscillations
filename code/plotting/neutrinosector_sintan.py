...
sns.kdeplot(x=np.power((alldf['sin_theta_13'].iloc[0:100000]),1),
            ...
            color='blue', fill=True, cut=0, clip=[(-1,1),(0,2)],
...
axs[0, 0].set_yticks(np.arange(5, 26, 5))
axs[0, 0].grid()
"""
...
sns.kdeplot(x=np.power(df['sin_theta_13'],1), color='#4080bf',
            ax=axs[0, 0], clip=(-1,1), label=r'KDE')

axs[1, 1].set_xlabel(r"Density of occurences")
axs[1, 1].set_xticks(np.arange(1, 6, 2))
axs[1, 1].grid()
...
axins = inset_axes(axs[1, 0], width='45%', height='45%', loc=1, borderpad=1.5)
axins.set_xlim(-0.04, 0.04)
axins.set_ylim(0.8, 1.2)
sns.kdeplot(x=np.power((alldf['sin_theta_13'].iloc[0:100000]),1),
            y=alldf['tan_theta_12'], clip=[(-1,1),(0,2)],
            color='blue', fill=True, cut=0,
            cmap="Blues", ax=axins)
axins.set_xlabel('')
axins.set_ylabel('')
axins.grid()
mark_inset(axs[1, 0], axins, loc1=2, loc2=4, fc="none", ec="0.5")
